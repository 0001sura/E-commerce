...
//   private userService:UserService,
 ...
 async addReview(currentUser: UserEntity,productId,createReviewDto: CreateReviewDto):Promise<Review> {
//   const user=await this.userService.findOne(userId)
 ...
 async reviewUpdate(userId: number, reviewId:number, updateReviewDto: UpdateReviewDto) {
  ...
  async removeReview(reviewId: number, currentUser:UserEntity):Promise<DeleteResult> {
  ...
  async getProductReview(productId:number) {
    return await this.reviewRepository.find({where:{products:{id:productId}}, relations:{user:true}});
  }
}
